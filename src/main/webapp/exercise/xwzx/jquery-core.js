...
/**
 * 扩展ajax提交  如果头部存在jrfAdminLogin_status ==timeout  说明前端登录超时  刷新页面重新登录  mxs 20171117
 */
var ajax = $.ajax;
$.ajax = function (opt) {

	var isIEVersion = function(){
	...
	if(!isIEVersion()){
		// ie 6\7\8\9 环境下清除 crossDomain 和 xhrFields属性;
		delete opt.crossDomain;
		delete opt.xhrFields;
	}
    ...
    var _opt = $.extend(opt, {
        success: function (data, textStatus, jqXHR) {
            if(jqXHR.getResponseHeader("jrfAdminLogin_status") && jqXHR.getResponseHeader("jrfAdminLogin_status") == "timeout"){
                window.location.href = "/jrfadmin/jrfcms/login.do"
                //window.location.reload(true);//刷新当前页
                return;
            }
            try{fn.success(data, textStatus, jqXHR);}catch(e){}
        }
    });
...
Date.prototype.pattern=function(fmt) {
    ...
    if(/(e+)/.test(fmt)){
        fmt=fmt.replace(RegExp.$1, weekS[this.getDay()+""]);
    }
    return fmt;
}
